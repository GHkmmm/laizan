...
import { onMounted, onBeforeUnmount, ref, h, watchEffect, nextTick } from 'vue'
import {
  NButton,
  NInput,
  NInputNumber,
...
const start = async (): Promise<void> => {
  ...
  try {
    ...
    if (result.ok) {
      ...
      addLog(`启动失败: ${result.message || '未知错误'}`)
    }
  } catch (error) {
    taskStatus.value = 'idle'
    addLog(`启动异常: ${String(error)}`)
  }
}
...
const stop = async (): Promise<void> => {
  ...
  try {
    ...
    if (!result.ok) {
      taskStatus.value = 'running'
      addLog(`停止失败: ${result.message || '未知错误'}`)
    }
  } catch (error) {
    taskStatus.value = 'running'
    addLog(`停止异常: ${String(error)}`)
  }
}
...
<template>
  <div class="relative flex flex-col gap-4">
    <div v-if="hasAuth" class="absolute top-4 right-2">
      <n-button type="primary" tertiary @click="logout">退出登录</n-button>
    </div>
